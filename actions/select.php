...
EOF;
 ...
 {  
	  if($rows > 10)
	  {
		  ...
		  $delete_sql = "DELETE FROM components LIMIT $delete_records";
      ...
      {  
           $output .= '  
                <tr>  
                     ...
                     <td class="cost" data-id2="'.$row["id"].'" contenteditable>'.$row["cost"].'</td> 
					 <td class="quantity" data-id3="'.$row["id"].'" contenteditable>'.$row["quantity"].'</td> 
                     <td><button type="button" name="delete_btn" data-id4="'.$row["id"].'" class="btn btn-xs btn-danger btn_delete">x</button></td>  
					 <td><button type="button" name="issue_btn" data-id5="'.$row["id"].'" class="btn_issue">issue</button></td>  
