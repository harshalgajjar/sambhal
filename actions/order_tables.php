<?php
  ...
  {
    $sql = "select distinct r.id as id,s.roll_no as roll_no,r.name as name,r.type as type,r.quantity as quantity,r.cost as cost,r.cause as reason from student as s, request as r where r.student_id = s.id and r.status = 'Approval Pending' order by id;";
    $result = pg_query($db, $sql);
    $rows = pg_num_rows($result);
    $output = '';
    $output .= '<head><h3><u>Approvals Pending</u></h3></head><br>';
    if($rows > 0)
    {
      $output .= "<table class=\"result-table\" id=\"request-pending-table\">
      <thead>
      <tr>
    ...
    $query = "select distinct r.id as id,s.roll_no as roll_no,r.status as status,r.name as name,r.type as type,r.quantity as quantity,r.cost as cost,r.cause as reason from student as s, request as r where r.student_id = s.id and r.status in ('Approved','Declined') order by id;";
    $result = pg_query($db, $query);
    $rows = pg_num_rows($result);
    $output .= '<head><h3><u>Approved/Declined</u></h3></head><br>';
    if($rows > 0)
    {
      $output .= "<table class=\"result-table\" id=\"request-table\">
      <thead>
      <tr>
      <th rowspan=2>S.no.</th>
      <th rowspan=2>Roll Number</th>
    ...
    {
      $output .= "<span class=\"no_results\"><h3> No Approvals/Declines </h3> </span>";
    }
  ...
  if($_SESSION['level']=="staff") {
    ...
    $rows = pg_num_rows($result);
      $vomit = '';
    $vomit .= '<head><h3><u>Purchase Pending</u></h3></head><br>';

    if($rows > 0)
    {
      $vomit.= "<table class=\"result-table\" id=\"order-table\">
      <thead>
      <tr>
      <th rowspan=2>S.no.</th>
    ...
    {
        $vomit.= "<span class=\"no_results\"><h3> No Orders </h3> </span>";
    }
    echo $vomit;
  }
